...
func getMessageWithRetries(primary, secondary, tertiary string) ([3]string, [3]int) {
	// ?
	res :=[3]string{primary,secondary,tertiary}
	resi:=[3]int{len(primary),len(primary)+len(secondary),len(primary)+len(secondary)+len(tertiary)}
	return res,resi
}

func main(){
	s,i:=getMessageWithRetries("hello","bye","add")
	fmt.Println(s)
	fmt.Println(i)
}
